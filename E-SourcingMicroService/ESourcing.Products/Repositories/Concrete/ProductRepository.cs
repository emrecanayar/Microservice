...
namespace ESourcing.Products.Repositories.Concrete
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task<IEnumerable<Product>> GetProdutcs()
        {
        ...
        public async Task<IEnumerable<Product>> GetProdutcsByName(string name)
        {
            var filter = Builders<Product>.Filter.ElemMatch(p => p.Name, name);
            return await _context.Products.Find(filter).ToListAsync();
        }

        public async Task<IEnumerable<Product>> GetProdutcsCategory(string categoryName)
        {
